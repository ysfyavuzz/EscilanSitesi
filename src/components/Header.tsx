        ...
        <nav className="hidden md:flex items-center gap-10">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href}>
              <span className={`text-[10px] font-black uppercase tracking-[0.2em] cursor-pointer transition-colors hover:text-primary
                ${location === link.href ? 'text-primary' : (isDark ? 'text-white/40' : 'text-orange-950/40')}`}>
                {link.label}
              </span>
            </Link>
          ))}
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          <div className="hidden md:flex items-center gap-4">
            {isAuthenticated ? (
              ...
              <Link href="/login">
                <button className="bg-primary text-white px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest italic shadow-xl shadow-primary/20 hover:scale-105 transition-all">
                  YÖRÜNGEYE GİR
                </button>
              </Link>
            )}
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-full left-0 right-0 glass-panel border-none m-6 p-8 flex flex-col gap-6 md:hidden shadow-2xl"
          >
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href}>
                <span onClick={() => setMobileMenuOpen(false)} className={`text-xs font-black uppercase tracking-widest italic ${location === link.href ? 'text-primary' : 'text-white/60'}`}>
                  {link.label}
                </span>
              </Link>
            ))}
            ...
            {!isAuthenticated ? (
              <Link href="/login">
                <button onClick={() => setMobileMenuOpen(false)} className="w-full bg-primary text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest italic">
                  YÖRÜNGEYE GİR
                </button>
              </Link>
            ) : (
